      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        ...
        <header className="flex items-center justify-between mb-12">
          ...
          <nav className="flex gap-2">
            {pendingBets.length > 0 && (
              <Link href="/dashboard/active" className="px-4 py-2 text-sm rounded-lg bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 transition">
                {pendingBets.length} en cours
              </Link>
            )}
            <Link href="/dashboard/sessions" className="px-4 py-2 text-sm rounded-lg bg-zinc-800 text-zinc-300 hover:bg-zinc-700 transition">
              Sessions
            </Link>
          </nav>
        </header>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/sessions" className="text-zinc-500 text-sm hover:text-white transition">Voir tout →</Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {recentSessions.map((session) => {
              ...
              return (
                <Link key={session.id} href={`/dashboard/sessions/${session.id}`} className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-4 hover:border-zinc-700 transition">
